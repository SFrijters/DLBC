...
#    plt.show()
 ...
 #   eps_bar = 1.0/(1.6*math.pi)
    ...
    D_norm = D / ( eps_bar * E0 * E0 * R / sigma )
...
ax.set_ylabel(r"$D (\times 10^{-3})$",fontsize=talfs, family=tff)
...
write_to_file("test-droplet-zoom-unscaled")
...
D_max = [ 0.01 for e in list(gamma_t) ]
p, = ax.plot(gamma_t, D_max, ls="None", marker="None", c='k',label=r"$D = 0.01$")
p.set_dashes([4,4])
...
# Styles
ax.set_xlabel(r"$\gamma$",fontsize=talfs, family=tff)
ax.set_ylabel(r"$D$",fontsize=talfs, family=tff)
...
write_to_file("test-droplet-unscaled")
