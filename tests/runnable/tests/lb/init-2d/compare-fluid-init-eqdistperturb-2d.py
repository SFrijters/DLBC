...
globstrr = os.path.join(relpath, "population-red*fluid-init-eqdistperturb*h5")
gr = glob.glob(globstrr)
fr = h5py.File(gr[0], 'r')
vr = fr["/OutArray"]
totalr = np.sum(vr)
# Random values should not deviate too far from average value.
relchanger = abs(1 - ( totalr / (0.5 * 64 * 64)) )
...
globstrb = os.path.join(relpath, "population-blue*fluid-init-eqdistperturb*h5")
gb = glob.glob(globstrb)
fb = h5py.File(gb[0], 'r')
vb = fb["/OutArray"]
totalb = np.sum(vb)
# Random values should not deviate too far from average value.
relchangeb = abs(1 - (totalb / (1.0 * 64 * 64)) )
